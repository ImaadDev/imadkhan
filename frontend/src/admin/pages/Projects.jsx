...
const ProjectsPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (modalMode === 'add') {
        const response = await axios.post(`${BackendUrl}/api/projects`, projectData);
        setProjects(prev => [...prev, response.data]);
        showAlert('[SUCCESS]', 'Project created successfully.');
      } else if (modalMode === 'edit') {
        const response = await axios.put(`${BackendUrl}/api/projects/${currentProject._id}`, projectData);
